    ...
    public class BookingService {
        ...
        public ResponseEntity<Object> getBookingDetails(
            ...
            try {
                ...
                try {
                    String paymentUrl = paymentService.getPaymentUrl(bookingCode);
                    if (paymentUrl != null && !paymentUrl.isEmpty()) {
                        RespCreateTransactionDTO paymentResponse = new RespCreateTransactionDTO();
                        paymentResponse.setTransactionCode(bookingCode);

                        Map<String, Object> paymentData = new HashMap<>();
                        paymentData.put("invoiceUrl", paymentUrl);
                        paymentResponse.setPaymentData(paymentData);

                        response.setRespCreateTransactionDTO(paymentResponse);
                        log.info("Payment URL added for booking: {}", bookingCode);
                    }
                } catch (Exception e) {
                    log.warn("Could not fetch payment URL for booking: {} - {}", bookingCode, e.getMessage());
                }
                ...
                if (request.getEquipment() != null) {
                    for (EquipmentSelectionRequest equipmentRequest : request.getEquipment()) {
                        ...
                        equipmentResponses.add(EquipmentDetailResponse.builder()
                                ...
                                .brand(equipment.getBrand())
                                .type(equipment.getType())
                                .categoryName(equipment.getEquipmentCategory() != null ? equipment.getEquipmentCategory().getCategoryName() : "")
