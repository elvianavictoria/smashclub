...
public class ReqAdminEquipmentSaveDTO extends CustomRequestValidation {
    ...
    public void validate() {
        ...
        else if( !CustomRegex.stringRegex(equipmentName, 3, 50) ) {
            validationObject.add(constructValidationItem("equipmentName", equipmentName, "Equipment name length must be between 3 to 50 and alphanumeric!"));
        }
        ...
        // Need regex validation and length
        else if( !CustomRegex.stringRegex(brand, 3, 50) ) {
            validationObject.add(constructValidationItem("brand", brand, "Brand length must be between 3 to 50 and alphanumeric!"));
        }
        ...
        // Need regex validation and length
        else if( !CustomRegex.stringRegex(type, 3, 30) ) {
            validationObject.add(constructValidationItem("type", type, "Type length must be between 3 to 30 and alphanumeric!"));
        }
